...
from catalog.models import Company, Month, TypeOfJobs, Department, HeadOfDepartment, Employee
...
class AdminSiteDeleteObjectTestCase(BaseAdminSiteTestCaseMixin, TestCase):
    ...
    def test_delete_employee(self):
        self.check_delete_object(model=Employee, app_name=CatalogConfig.name)

    def test_delete_department(self):
        self.check_delete_object(model=Department, app_name=CatalogConfig.name)

    def test_delete_headofdepartment(self):
        self.check_delete_object(model=HeadOfDepartment, app_name=CatalogConfig.name)
